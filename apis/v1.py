...
class RelationCreateView(BaseView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return self.response(message='잘못된 요정입니다.', status=400)
...
class UserLoginView(BaseView):
    def post(self, request):
        ...
        user = authenticate(request, username=username, password=password) # authenticate 는 아이디나 패스워드를 잘못입력하면 None를 반환해준다.
        if user is None:
